...
namespace Lab12
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            Player Oppenent = RoshamboApp.ChooseOpponent();
            PlayGame(ref wins, ref losses, UserName, ref Oppenent);
            Console.WriteLine($"You won {wins} and lost {losses}");
        }
        #endregion
        #region Game!
        public static void PlayGame(ref int wins, ref int losses, Human UserName, ref Player Oppenent)
        {
            while (true)
            {
                Roshambo Input = UserName.generateRoshambo();
                Roshambo AI = Oppenent.generateRoshambo();

                if (Oppenent.Name == "Dr. K")
                {
                    Counter(ref wins, ref losses, UserName, Oppenent, Input, AI);
                }
                if (Oppenent.Name == "Grant Chirpus")
                {
                    Counter(ref wins, ref losses, UserName, Oppenent, Input, AI);
                }
                ...
                if (Validate.Continue() == true)
                {
                    if (Validate.NewOpponent() == true)
                        Oppenent = RoshamboApp.ChooseOpponent();
                    Console.Clear();
                }
                else
                    break;
        ...
        public static void Counter(ref int wins, ref int losses, Human UserName, Player Oppenent, Roshambo Input, Roshambo AI)
        {
            Console.WriteLine($"{UserName.Name}: {Input}\n{Oppenent.Name}:{AI.ToString()}");
